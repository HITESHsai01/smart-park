...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      {showError && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-top">
          <div className="bg-red-50 border border-red-200 text-red-800 px-6 py-3 rounded-lg shadow-lg flex items-center gap-3">
            <XCircle size={20} />
            <p className="font-medium">Access Denied: You must be an owner to access that page.</p>
            <button
      ...
      {/* Header */}
      <header className="px-6 h-16 flex items-center justify-between border-b border-gray-200 dark:border-zinc-800 bg-white/50 dark:bg-black/50 backdrop-blur-sm sticky top-0 z-50">
        ...
        <nav className="flex items-center gap-4">
          <Link href="/owner/sign-up" className="text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-black dark:hover:text-white">
            Become a Partner
          </Link>
          <Link 
            href="/sign-in" 
      ...
      <main className="flex-1">
        <section className="py-20 px-6 text-center max-w-4xl mx-auto space-y-8">
          ...
          <p className="text-xl text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
            The smartest way to park. Book spots instantly, manage your listings, and save time with SmartPark.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
            <Link
              href="/map"
              className="w-full sm:w-auto px-8 py-4 rounded-full bg-blue-600 text-white font-semibold text-lg hover:bg-blue-500 transition-colors flex items-center justify-center gap-2"
            >
              Find a Spot <ArrowRight size={20} />
            </Link>
            <Link
              href="/owner/sign-up"
              className="w-full sm:w-auto px-8 py-4 rounded-full bg-gray-100 dark:bg-zinc-800 text-gray-900 dark:text-white font-semibold text-lg hover:bg-gray-200 dark:hover:bg-zinc-700 transition-colors"
            >
              List Your Space
            </Link>
          </div>
